...
If you find a bug please email me at C<< <rschmieder_at_gmail_dot_com> >> so that I can make PRINSEQ better.
...
if(exists $params{out_good} && exists $params{out_bad} && $params{out_good} eq $params{out_bad} && $params{out_good} ne 'null') {
...
my ($repAleft,$repTleft,$repAright,$repTright);
...
close($fhbad) unless($nobad);
if($params{out_format} == 2) {
    close($fhgood2) unless($nogood);
    close($fhbad2) unless($nobad);
}
#remove empty files
if($seqcount == 0 && !$nogood) {
    unlink($filenamegood);
    if($params{out_format} == 2) {
...
sub processData {
    ...
    if($good && defined $qualn && $trimscore) {
        my $quals = &convertQualAsciiToNums($qualn);
        $length = scalar(@$quals);
        my $i = 0;
        my $begintmp = 0;
        my $endtmp = 0;
    ...
    #check for quality scores
    if($good && $qual && (exists $params{min_qual_score} || exists $params{max_qual_score} || exists $params{min_qual_mean} || exists $params{max_qual_mean})) {
        my $quals = &convertQualAsciiToNums($qualn);
	...
	if($numkmer > 1) {
		...
		if(!defined @{$matrix[0]->[$i]}) { #not matching
			...
			foreach my $j (0..$count) {
			    ...
			    if(defined $sign) {
				if(($sign < 0 && $matrix[$j]->[$i-1]->[0] < 0) || ($sign > 0 && $matrix[$j]->[$i-1]->[0] > 0)) {
				    $sum += $matrix[$j]->[$i-1]->[0];
